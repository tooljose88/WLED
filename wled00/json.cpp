...
void deserializeHA(JsonObject root){
  bri = root["brightness"] | bri;
  const char * state = root["state"];
  Serial.println(state);
  Serial.println(bri);
  Serial.print("BriLast");
  Serial.println(briLast);
  if(root.containsKey("state")){
    Serial.println("Estadito");
    if (!strcmp(root["state"], "ON") && bri==0) toggleOnOff();
    else if(!strcmp(root["state"], "OFF") && bri>0)toggleOnOff();
    else if (!strcmp(root["state"], "2")) {Serial.println("Entra");toggleOnOff();}
  }
  if (root.containsKey("color")){
    Serial.println("Colorcito");
    col[0] = root["color"]["r"];
    col[1] = root["color"]["g"];
    col[2] = root["color"]["b"];
  }
